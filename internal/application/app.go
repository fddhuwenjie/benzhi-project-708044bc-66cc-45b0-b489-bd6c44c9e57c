...
type App struct {
	...
	evidenceResponses *evidenceResponseCache
}

func New(s *persistence.Store) *App {
	return &App{Store: s, evidenceResponses: newEvidenceResponseCache()}
}
...
func (a *App) RetestCounts(id, caseID string, result domain.RetestResult, resolution, requestID string, expected int) (*domain.RejuvenationBatch, error) {
	...
	if _, err := a.Store.SaveAndRemember(b, "remediation_completed", payload, expected, remember); err != nil {
