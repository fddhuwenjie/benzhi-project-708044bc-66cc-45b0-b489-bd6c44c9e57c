...
func (s *Store) SaveAndRemember(b *domain.RejuvenationBatch, typ string, p any, expected int, req string) (*audit.Event, error) {
	return s.SaveAndRememberContext(context.Background(), b, typ, p, expected, req)
}

func (s *Store) SaveAndRememberContext(ctx context.Context, b *domain.RejuvenationBatch, typ string, p any, expected int, req string) (*audit.Event, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	if req == "" {
		event, err := s.saveLocked(b, typ, p, expected, "")
	...
	event, err := s.saveLocked(b, typ, p, expected, req)
	if err == nil && ctx.Err() != nil {
		return nil, ctx.Err()
	}
	return event, err
